...
export default function AdministrarTipoMovimiento() {
  ...
  async function handleEliminarMovimiento(id, nombre) {
    await Swal.fire({
      ...
      if (result.isConfirmed) {
         recursosService
          .destroyTipoMovimientoManejo(onMovimientoEliminado, id, onError)
          .then(() => {
            cargarDatos();
          });
      }
  ...
  const onSearch = (value) => {
    recursosService.showTipoMovimientoManejoSearch(value, setDatos, onError);
  };
  ...
  async function handleSubmitNuevo() {
    ...
    await Swal.fire({
      ...
      if (result.isConfirmed) {
        recursosService.createTipoMovimientoManejo(
